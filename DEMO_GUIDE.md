...
> 当前目标：v0.5 本地演示版 + 只读报告诊断 Agent + 待审批规则增强 + 整改闭环
> 最近验收日期：2026 年 7 月 29 日
> 固定示例报告基准日期：2026 年 7 月 17 日
> 运行环境：Python 3.10 或更高版本；当前完整验收版本为 3.12.4
> 验收结果：`python3 run_demo.py --check` 已运行 282 项测试且全部通过；固定示例报告与 v0.4 指标引擎一致，Streamlit 随机本机端口健康检查通过
> 核心边界：零配置报告是基础事实源；Agent 只读解释报告，业务规则只有经本地校验和用户明确批准后才由确定性 Python 引擎执行，模型不可用时完整闭环仍可运行。
...
11. v0.5 历史报告只有显式保存或导入后才进入当前会话；报告比较绑定两份完整哈希，同输入规则新增差异不能冒充整改效果。
12. 跨版本行动计划只引用确定性比较变化；不配置模型时，历史、比较、趋势、任务分派和导出仍可完成。
...
1. 上传 `good_dataset.csv`，将基准日期固定为 `2026-07-17` 并运行零配置评估；
...
5. 输入审批人标识，勾选确认后点击“批准并重新评估”，确认页面同时显示零配置与规则增强结果计数以及新增业务指标/风险；
...
1. 上传 `bad_dataset.csv`，把数据集名称固定为“政务服务整改样例”，将基准日期设为 `2026-07-17` 并运行评估；
2. 在“历史版本与整改闭环”区域填写稳定的治理对象标识和“整改前”版本标签，显式保存当前报告；确认系统说明历史只保存在当前会话内存；
3. 上传 `good_dataset.csv`，继续使用相同数据集名称、治理对象标识和基准日期运行评估，再以“整改后”保存；
4. 在两个历史版本选择器中依次选择“整改前”和“整改后”，显式确认它们属于同一治理对象后运行比较；
5. 查看指标改善/恶化、风险新增/解除、恢复或失去可评估、字段结构变化和版本趋势；确认每项变化都具有稳定 `change_id`，且没有综合分；
6. 生成本地整改计划，按需填写负责人、截止日期和任务状态；确认任务只引用比较变化，不包含原始字段值；
7. 下载比较 JSON、行动计划 JSON/Markdown/CSV 和治理记录 JSON；CSV 应可在表格软件中安全打开；
8. 删除单份历史并测试全部清空；确认删除不会影响当前 `QualityReport`，也不会修改或清洗任何上传文件；
9. 可另行导入此前下载的严格 `QualityReport` JSON，验证跨会话由用户自行保管报告、当前服务不做后台长期留存；同时确认界面明确说明结构校验和自哈希不是来源签名，也不能从语义上证明任意合法文本字段不含敏感内容，因此只应导入内容与来源可信的报告。

比较器对数据规模、字段新增/删除、字段类型变化及没有登记改善方向的指标只描述变化。若引擎、阈值、业务规则或指标定义口径不兼容，页面必须明确显示比较受限，不能输出无依据的“已改善/已恶化”。
...
- 未批准、校验失败、审批后被修改或与当前输入不匹配的 RulePack 均不能执行；增强结果只追加业务规则指标和风险，原 13 项指标、既有风险、默认阈值和零配置报告保持不变；
...
- 历史报告仅在用户显式保存或导入后进入当前会话内存；不保存上传字节、问题位置 CSV、Agent 对话或 RulePack，并受单份、数量和总内存上限约束；
- 历史 JSON 导入会严格拒绝非 UTF-8、重复键、`NaN` / `Infinity`、过深结构、Schema 不符、哈希失配、重复指标键以及坏风险/无法评估引用；
- 两份报告只有在治理对象标识一致且用户确认绑定两份报告哈希后才比较；比较器按稳定引用输出 `ReportComparison`，不生成综合分；
- 指标、风险、无法评估和字段结构变化具有稳定变化 ID；不可比口径只给出限制原因，不生成改善或恶化结论；
- 本地整改计划只从已验证变化生成任务；负责人、期限和状态由用户填写，行动计划与治理记录可独立导出；
- 单报告 Agent 工具白名单保持不变；v0.5 比较器和历史删除能力不注册为 DeepSeek 工具；
...
- 不提供服务器侧长期历史数据库、登录认证、多人权限隔离、周期调度或批量数据集治理；
- 不让外部模型创建或修改比较事实、删除历史、填写负责人/期限，或降低本地确定的最低任务优先级；
