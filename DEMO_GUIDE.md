...
> 当前目标：由 v0.4 直接演进的 v0.6 本地演示版 + 自由指标选择 + DB31/T 1523-2024 指标 + 只读报告诊断 Agent + 待审批规则增强
> 最近验收日期：2026 年 7 月 30 日
> 固定示例报告基准日期：2026 年 7 月 17 日
> 运行环境：Python 3.10 或更高版本；当前完整验收版本为 3.12.4
> 验收结果：270 项自动化测试、固定报告一致性与 Streamlit `AppTest` 已通过；`python3 run_demo.py --check` 还会执行真实本机随机端口健康检查
> 核心边界：当前所选指标报告是基础事实源；Agent 只读解释报告，业务规则只有经本地校验和用户明确批准后才由确定性 Python 引擎执行，模型不可用时完整闭环仍可运行。
...
4. 查看其他 DB31/T 指标：缺少数据标准、业务规则、权威参照、跨表关系、授权记录、访问 SLA 或应用场景时，结果明确为“无法评估”，并列出所需依据；
