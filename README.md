...
v0.6 **直接以 v0.4 为基础开发，不包含或继承 v0.5 的跨历史版本比较能力**。它完整保留 v0.4 的 13 项原有指标，并加入 DB31/T 1523-2024《公共数据质量评价要求》正文中的 30 项指标，形成 43 项可选目录。用户可自由组合指标，Demo 只计算和报告本次选中的指标；同义或重复指标以不同稳定 ID 同时保留，不互相覆盖。默认仍选择原 v0.4 的 13 项，便于复现原有结果，也可一键切换为“仅 DB31/T”或“全部指标”。
...
6. 在“Agent 解读”页签中按需生成报告概括、优先整改建议、无法评估项说明，或询问仅基于当前报告的问题。
