...
v0.6 **直接以 v0.4 为基础开发，不包含或继承 v0.5 的跨历史版本比较能力**。它完整保留 v0.4 的 13 项原有指标，并加入 DB31/T 1523-2024《公共数据质量评价要求》正文中的 30 项指标，形成 43 项可选目录。v0.7 在此基础上增加补充评价标准 Agent：用户可在指标卡片下填写自然语言依据；配置外部模型时由该模型生成受限规则草案，未配置时才使用本地模板作暂行演示，再经确定性校验、试运行和人工批准后重评。既有指标、风险和报告口径保持不变。
...
v0.7 新增“补充评价标准”页签。报告摘要会明确列出需要外部标准的指标及所需输入；用户可在该页补充依据，再按“AI 解析 → 确定性试运行 → 批准并重新评估”完成规则闭环。它只支持现有五类规则，模型只负责理解和编译；字段/参数校验、审批前试运行、审批和正式重评均由本地 Python 确定性代码完成。初始页面可填写 API 地址、API Key 和模型名称，调用兼容 OpenAI Chat Completions 格式的自定义模型；已配置外部模型时调用或解析失败会直接提示错误，绝不伪装成模板结果，并会列出当前依据缺少的字段、条件或参数。未配置 API Key 时才使用本地模板作暂行演示。RAG、LangGraph、Dify 和自然语言自定义规则不属于 v0.7。
...
8. 如需将业务依据编译为规则，在指标卡片下填写“评价依据”，报告生成后打开“补充评价标准”：点击“AI 解析依据”查看 RuleDraft，确认校验通过后点击“试运行规则”，最后填写审批人并明确批准。模糊依据会进入澄清；任意 Python / SQL / 脚本需求会被拒绝；若填写了页面 API 配置，规则编译也会调用对应模型。
