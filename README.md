...
一个本地运行的 Streamlit Demo，用于对 CSV、Excel（`.xls`、`.xlsx`）、表格型 JSON、JSON Lines、GeoJSON FeatureCollection 和同构 JSON 分片 ZIP 数据集生成可复现的质量指标、风险提示、疑似问题位置 CSV 与无法评估项。v0.5 在 v0.4 待审批 `RulePack` 之外，新增固定报告历史、确定性跨版本比较、版本趋势、可分派整改任务、行动计划与治理记录导出；所有改善或恶化结论均绑定两份完整报告哈希。
...
v0.5 的报告比较与整改任务默认完全在本地运行，不调用 DeepSeek。用户必须显式把当前报告保存到历史，或导入此前下载的严格 `QualityReport` JSON；系统不会因评估完成而自动留存。导入时会复核 UTF-8、JSON 深度、重复键、非标准数值、JSON Schema、报告哈希、隐私结构、指标状态、风险引用和无法评估引用，并拒绝非空原值样例数组、原始记录容器及未知画像/证据字段。结构校验无法从语义上证明任意合法文本字段绝不含敏感内容；报告自哈希也只能证明对象内部自洽，不是来源签名或提交人认证。因此只应导入当前 Demo 生成或已由导入者确认内容与来源可信的报告，且应预期其中的合法文本可能继续出现在比较或整改导出中。

当前历史仓库只存在于本次 Streamlit 会话内存：不落盘、不建数据库，不保存上传字节、问题位置 CSV、Agent 对话或 RulePack；保留到用户显式删除或会话终止为止。当前 Demo 没有登录或身份认证能力，访问范围仅是当前本机浏览器会话。每份历史报告最多 8 MiB，单会话最多 20 份、合计最多 32 MiB；如需跨会话治理，应下载结构化报告，并在下一次会话中重新导入。

比较前必须由用户为两份固定报告声明相同的“治理对象标识”。比较器按 `metric_key`、`risk_id`、无法评估项引用和字段名进行确定性对齐；指标引擎或阈值口径变化、业务规则证据不一致、指标被删除等情况会明确标记为受限或不可判断，不会硬凑“已改善”。数据规模、字段增删、字段类型变化和未登记方向的指标只描述变化，不自动判断好坏。

跨版本整改任务由本地模板依据已验证的 `change_id` 生成并排序，不接触原始文件，也不重新计算指标。负责人、截止日期和任务状态由用户填写；系统可导出权威比较 JSON、行动计划 JSON/Markdown/CSV 和独立治理记录。现有单报告 `AgentAnalysis`、v0.4 `RulePack`、v0.5 `ReportComparison` 与整改计划始终彼此独立。
...
7. 在“历史版本与整改闭环”区域显式保存当前报告或导入历史 JSON；为同一治理对象选择整改前后版本，运行确定性比较，生成、分派并导出整改任务和治理记录。
...
- 规则增强只追加确定性业务规则结果，不修改原 13 项指标、既有风险或默认阈值；v0.4 的同输入“新增规则项”仍不得冒充 v0.5 的跨历史整改效果；
- `ReportComparison` 由本地确定性比较器生成并绑定两份完整报告哈希；任何改善、恶化、风险解除或失去可评估结论都必须引用比较变化 ID；
- 历史报告仅在用户显式保存或导入后进入有界会话内存，支持单条删除和全部清空；默认不做服务器侧长期保存；
- 整改计划只引用已验证的比较变化；模型不可用或未配置时，比较、版本趋势、任务生成、分派和导出仍完整可用；
