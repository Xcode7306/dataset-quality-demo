...
v0.6 **直接以 v0.4 为基础开发，不包含或继承 v0.5 的跨历史版本比较能力**。它完整保留 v0.4 的 13 项原有指标，并加入 DB31/T 1523-2024《公共数据质量评价要求》正文中的 30 项指标，形成 43 项可选目录。v0.7 在此基础上增加补充评价标准 Agent；v0.8 新增自然语言自定义规则入口，支持格式/正则、字符长度、条件必填和跨字段比较。规则仍经确定性校验、试运行和人工批准后重评，既有指标、风险和报告口径保持不变。
...
v0.7 新增“补充评价标准”页签；v0.8 在同一审批链中新增“自定义规则”入口。自定义规则支持格式/正则、字符长度、条件必填和跨字段比较，范围外的跨表参照、任意 Python/SQL、自动清洗和数据写回会明确拒绝。模型只负责理解和编译；字段/参数校验、审批前试运行、审批和正式重评均由本地 Python 确定性代码完成。初始页面可填写 API 地址、API Key 和模型名称，调用兼容 OpenAI Chat Completions 格式的自定义模型；已配置外部模型时调用或解析失败会直接提示错误，绝不伪装成模板结果。未配置 API Key 时才使用本地模板作暂行演示。RAG、LangGraph 和 Dify 仍未接入。
...
8. 如需将业务依据编译为规则，在指标卡片下填写“评价依据”，报告生成后打开“补充评价标准”；也可在同页“自定义规则（v0.8）”输入自然语言需求。点击解析查看 RuleDraft，确认校验通过后试运行，最后填写审批人并明确批准。模糊依据会进入澄清；跨表参照、任意 Python / SQL / 脚本和自动修复会被拒绝；若填写了页面 API 配置，规则编译也会调用对应模型。
